...
try:
    ...
    email.To = 'mrcartonrex@gmail.com'
    email.Subject = f'Analises dos Dados de entregas na Cidade de Curitiba' #assunto
    email.HTMLBody = f""" 
    <p>Bom dia</p>
    <p>Segue os resultados da analise dos dados coletados das entregas na cidade de Curitiba no periodo de <strong> 3 meses</strong>: </p>
    <p>Os gráficos comparativos foram anexados a este e-mail.</p>
    """

    if pasta_imagens.exists() and pasta_imagens.is_dir():
        for arquivo in pasta_imagens.iterdir():
            if arquivo.is_file() and arquivo.suffix == '.png':
                caminho_imagens = str(arquivo.resolve())
                email.Attachments.Add(caminho_imagens)
                print(f"Anexado: {arquivo.name}")
    else:
        print("Aviso: A pasta 'imagens' não foi encontrada para anexar os arquivos.")
